...
const AdminPrediction = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://backend-brain-2.onrender.com/predict', {
        method: 'POST',
        body: formData,
      });
  ...
  useEffect(() => {
    const fetchPredictions = async () => {
      try {
        const response = await fetch("https://backend-brain-2.onrender.com/history");
  ...
  const handleDeletePrediction = async (id) => {
    try {
      const res = await fetch(`https://backend-brain-2.onrender.com/delete-history/${id}`, {
        method: "DELETE",
      });
