...
const Prediction = () => {
  ...
  const refreshCaptcha = () => {
    setUserCaptcha("");
    setCaptchaImg(`https://backend-brain1-beqb.onrender.com/generate-captcha?${Date.now()}`);
    setShowCaptcha(true);
    setOtpSent(false);
  };
  ...
  const handleCaptchaSubmit = async () => {
    setVerifying(true);
    try {
      const res = await fetch("https://backend-brain1-beqb.onrender.com/verify-captcha", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        const otpRes = await fetch("https://backend-brain1-beqb.onrender.com/send-otp", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
